<template>
    <div :class="[
        'fa-table',
        {
            'fa-table__striped': stripe
        }
    ]" ref="table">
        <div class="fa-table__table-wrapper">
            <div class="fa-table__table-container" ref="container">
                ...
                <!-- header -->
                <div class="fa-table__header-wrapper" ref="header" 
                ...
                <div class="fa-table__body-wrapper" ref="body"
                    :style="{'overflow':'auto',height: bodyHeight ? bodyHeight + 'px' : 'auto'}"
                >
                    <table-body
                    ...
                    <div v-else 
                        :style="{ width: bodyWidth + 'px' }" 
                        :class="{
                            'fa-table__empty-data': true,
                            'fa-table__empty-border': border
                        }">
                        <slot name="empty">
                            <div class="fa-table__empty-default">
                                <empty-icon></empty-icon>
                                <span class="fa-table__empty-text" v-text="emptyText"></span>
                            </div>
                        </slot>
                    </div>
                </div>
                ...
                <!-- footer -->
                <div class="fa-table__footer-wrapper" ref="footer"
                    :style="{width: isContainerScroll ? bodyWidth + 'px' : '100%'}"
                >
                    <table-footer
                        v-if="showSummary" 
                ...
                <div :class="[
                    'fa-table-fixed',
                    'fa-table__fixed-left',
                    {
                        'fa-table-fixed__left-empty-border': border && !data.length,
                        'fa-table-fixed__left-with-border': border && data.length
                    }
                ...
                :style="[{width: fixedLeftWidth + 'px', marginLeft: selectionColumn[0] ? selectionColumn[0].width + 'px' : 0}, fixedBottom]">
                    <!-- header -->
                    <div class="fa-table-fixed__header-wrapper" ref="leftHeader">
                        <table-header
                    ...
                    <!-- footer -->
                    <div class="fa-table-fixed__footer-wrapper" ref="leftFooter">
                        <table-footer 
                            v-if="showSummary" 
                            v-show="data && data.length > 0"
                            :style="{ width: fixedLeftWidth + 'px' }" 
                ...
                <div :class="[
                    'fa-table-fixed',
                    'fa-table__fixed-right',
                    {
                        'fa-table-fixed__right-empty-border': border && !data.length,
                        'fa-table-fixed__right-with-border': border && data.length
                    }
                ]" v-if="rightColumns.length > 0"
                :style="[{width: (fixedRightWidth + 1) + 'px'}, fixedBottom]">
                     <!-- header -->
                    <div class="fa-table-fixed__header-wrapper" ref="rightHeader">
                        <table-header
                    ...
                    <!-- footer -->
                    <div class="fa-table-fixed__footer-wrapper" ref="rightFooter">
                        <table-footer 
                            v-if="showSummary" 
                            v-show="data && data.length > 0"
                            type="right" 
                ...
                <!-- Table loading -->
                <div class="fa-table__data-loading" v-if="loading">
                    <slot name="loading">
                        <div class="fa-table__loading-spinner">
                            <svg viewBox="25 25 50 50" class="circular"><circle cx="50" cy="50" r="20" fill="none" class="path"></circle></svg>
                        </div>
                    </slot>
                </div>
            </div>
            <div class="fa-table__pagination-box" v-if="shownPagination" v-show="total > 0">
                <div class="pagination-text-info" v-if="paginationInfo.text" v-html="paginationInfo.text"></div>
                <div class="fa-table__pagination" @click="changeCurPage">
                    <span 
                        :class="[
                            'page prev-page',
                            {
            ...
            updateColumnsWidth (columns = undefined) {
                ...
                if (!columns) {
                    allColumns = this.$slots.default
                        .filter(column => column.componentInstance && column.componentInstance.$options.name === 'fa-table-column')
