<template>
  <div
    class="fa-tree"
    :class="{
      'fa-tree--highlight-current': highlightCurrent,
  ...
  >
    ...
    <div class="fa-tree__empty-block" v-if="!root.childNodes || root.childNodes.length === 0">
      <span class="fa-tree__empty-text">{{ emptyText }}</span>
    </div>
    <div
      v-show="dragState.showDropIndicator"
      class="fa-tree__drop-indicator"
      ref="dropIndicator">
    </div>
  </div>
</template>
...
<script>
  ...
  import faTreeNode from './tree-node.vue';
  ...
  export default {
    ...
    methods: {
      ...
      handelKeydown(ev) {
        ...
        if (currentItem.className.indexOf('fa-tree-node') === -1) return;
        ev.preventDefault();
    ...
    created() {
      ...
      this.$on('tree-node-drag-over', (event, treeNode) => {
        ...
        const targetPosition = dropNode.$el.querySelector('.fa-tree-node__expand-icon').getBoundingClientRect();
